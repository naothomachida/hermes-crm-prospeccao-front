...
export const CompaniesPage: React.FC<CompaniesPageProps> = ({ companies, onEdit, onDelete, onDuplicate }) => {
  return (
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-50">
              <th className="px-8 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400">Entidade / CNPJ</th>
              <th className="px-8 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400">Responsáveis</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {companies.length > 0 ? companies.map(c => (
                ...
                <td className="px-8 py-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[9px] text-gray-400 font-bold uppercase tracking-tighter">{c.cnpj || 'CPF/Pessoa Física'}</div>
                    </div>
                  </div>
                </td>
                <td className="px-8 py-5">
                    ...
                    {c.contacts?.length > 0 ? (
                      c.contacts.map((contact: any, i: number) => (
                        <div key={i} className="text-[9px] font-bold text-slate-500 uppercase flex items-center gap-2">
                          <span className="text-indigo-600">●</span> {contact.name} ({contact.dept})
                        </div>
                      ))
                    ) : (
                      <span className="text-[9px] text-slate-300 italic uppercase">Sem contatos extras</span>
                    )}
                  </div>
                </td>
                <td className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{c.city || 'N/A'}</td>
                <td className="px-8 py-5">
                  <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); onDelete(c.id, 'companies'); }} className="p-1.5 text-red-600 hover:bg-red-50 rounded-md">
              ...
              <tr><td colSpan={4} className="px-8 py-12 text-center text-gray-400 text-[10px] font-black uppercase tracking-widest">Nenhuma entidade cadastrada</td></tr>
            )}
          </tbody>
        </table>
      </motion.div>
